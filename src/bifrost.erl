...
-export([start_link/2, establish_control_connection/3, await_connections/2,parse_address/1,parse_ip/1]).
...
init([HookModule, Opts]) ->
    ...
    SslCert = config:get_opt(ssl_cert, Opts),
    ...
    case listen_socket(Port, [{active, false}, {reuseaddr, true}, list]) of
        {ok, Listen} ->
			...
			?DEBUG_F("~p --- ~p server starting ,host:~p,port:~p  ~n",[?MODULE,?MODULE,IpAddress,Port]),
            proc_lib:spawn_link(?MODULE,
                                await_connections,
                                [Listen, HookModule:init(InitialState, Opts)]),
            {ok, {listen_socket, Listen}};
        {error, Error} ->
			?DEBUG_F("~p -- listen_socket,error:~p ~n",[?MODULE,Error]),
            {stop, Error}
    end.
...
terminate(_Reason, {listen_socket, Socket}) ->
	?DEBUG_F("~p -- listen_socket close,error:~p ~n",[?MODULE,_Reason]),
    gen_tcp:close(Socket);
terminate(_Reason, _State) ->
	?DEBUG_F("~p -- terminate,error:~p ~n",[?MODULE,_Reason]),
    ok.
...
control_loop(SrvPid, HookPid, {SocketMod, RawSocket} = Socket, State) ->
    case SocketMod:recv(RawSocket, 0) of
        {ok, Input} ->
			...
			?INFO_F("Command-> ~p,~p ~n",[Command, Arg]),
...
data_connection(ControlSocket, State) ->
    ...
    case establish_data_connection(State) of
        ...
        {error, Error} ->
			?DEBUG_F("~p --- error ~p ~n",[?MODULE,Error]),
            respond(ControlSocket, 425),
            throw(Error)
    end.
                    ...
                    respond(ControlSocket,
                            ...
                            lists:flatten(
                              io_lib:format("Entering Passive Mode (~p,~p,~p,~p,~p,~p)",
                                            [S1,S2,S3,S4,P1,P2]))),
					?DEBUG_F("~p --- Ip:~p,Port:~p ~n",[?MODULE,Ip,Port]),
                    {ok,
...
ftp_command(_, Socket, State, port, Arg) ->
    case parse_address(Arg) of
        {ok, {Addr, Port}} ->
			?INFO_F("~p --- Addr:~p,Port:~p ~n",[?MODULE,Addr,Port]),
            respond(Socket, 200),
...
ftp_command(Mod, Socket, State, cwd, Arg) ->
    case Mod:change_directory(State, Arg) of
        {ok, NewState} ->
            respond(Socket, 250, "directory changed to \"" ++ NewState#connection_state.current_dir ++ "\""),
            {ok, NewState};
        {error, _} ->
...
% Taken from jungerl/ftpd
parse_ip(Ip) ->
	Temp = erlang:atom_to_list(Ip),
	Tokens = string:tokens(Temp,"."),
	[S1,S2,S3,S4] = Tokens,
	{list_to_integer(S1),list_to_integer(S2),list_to_integer(S3),list_to_integer(S4)}.
